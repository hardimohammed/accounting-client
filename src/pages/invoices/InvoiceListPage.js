            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {invoices.map((inv, i) => {
                  ...
                  // Comparing against `new Date()` (the exact current
                  // instant) meant an invoice due "today" showed as
                  // overdue the moment any time passed midnight — a
                  // same-day due date should stay current for the
                  // whole day it's due, only overdue once that date
                  // has fully elapsed. Compare date-only strings instead.
                  const todayStr = new Date().toISOString().slice(0, 10);
                  const isOverdue = inv.status !== 'paid' &&
                    String(inv.due_date).slice(0, 10) < todayStr && balance > 0;
                  return (
                    <tr key={i}
